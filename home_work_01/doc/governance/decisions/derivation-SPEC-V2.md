...
- **DV-13 Radar 對齊 oracle**。裁決：每個影像像素與地圖對其經緯度的投影相距 ≤ 1 km；參考點＝產品範圍四角、四邊中點、中心＋本島內 ≥ 3 點；zoom 7 與 10；自動化可重現。依據：產品像素 ≈ 185 m（6°／3600 px），縣層級使用；DA 計算把等經緯度影像線性貼在 Mercator lat 20.5–26.5 的最大誤差 ≈ 3.8 km（lat 23.55 附近；lat 22.0 為 2.8 km、25.0 為 2.9 km），即預設 `L.imageOverlay` 作法 FAIL——這正是 S-11 所稱「實質偏移／重投影問題」，故 oracle 必須排除它；1 km 是 5 個原生像素、在 zoom 10 約 7.1 CSS px（23.5°N；lat 20.5–26.5 為 6.98–7.31 px〔DV-23 更正 2026-09-26：原文「約 2.8 CSS px」有誤〕），可量測且對縣層級脈絡無明顯錯位。重投影技術或地圖 CRS 仍 HOW。影響：RAD-5、AC-V2-19。
  **DV-13 補充（v2.1，acceptor 要求的比例性複核；oracle 保留為 ≤ 1 km）**。問題：1 km 是否對「有意義的地理對齊」過嚴，因為它迫使實作做重投影／CRS 工作（線性貼圖已知偏差約 3.8 km）。DA 複核：(1) **工作量由接受語義決定，不由數字決定**——S-11 明文「不預先接受實質偏移為已知限制…明顯錯位不可」並把系統性偏移／重投影問題列為 route DA 的事項，所以「不能用線性貼圖」是 acceptor 已接受的結果；任何 < 3.8 km 的容差都同樣迫使處理緯度非線性，任何 ≥ 3.8 km 的容差都等於預先接受實質偏移（在 zoom 10 為 ≈ 27 CSS px、zoom 12 為 ≈ 108 px 的可見錯位〔DV-23 更正：原文 10.6／43 px 有誤〕），違反 S-11。(2) **1 km 對正確處理不構成額外負擔**——逐列重採樣或改用等經緯度 CRS 的殘差在一個原生像素（≈ 185 m）量級；分段線性近似的殘差（DA 重算）：兩段 ≈ 1.02 km（略超出）、三段 ≈ 0.46 km、四段 ≈ 0.26 km，所以 1 km 允許三段以上的簡單近似解法，不強迫逐像素精確方案。(3) **可量測性**——1 km 在 zoom 10 約 7.1 CSS px（23.5°N），瀏覽器量測可清楚分辨；更寬的容差（例如 2 km）不減少任何工作，只會放寬對粗糙近似的把關；更嚴的容差（例如 200 m）在 zoom 10 約 1.4 px，落在渲染像素量測 1–2 px 的量化極限、且與正確作法本身約一個原生像素（≈ 185 m，第 (2) 點）的殘差同量級，無法可靠地與實作殘差區分〔DV-23 更正 2026-09-26：原文「約 2.8 CSS px」與「不足 1 px、無法可靠量測」有誤；本點與本補充的結論（保留 ≤ 1 km）不變〕。(4) 與縣層級使用一致：雷達產品像素 185 m，縣內站距通常 1–10 km，1 km 的錯位不會把回波錯置到相鄰縣或相鄰測站。**裁決：保留 ≤ 1 km；不需 acceptor**——改變容差只有兩種方向：放寬到 ≥ 3.8 km 會改變 accepted 產品品質（須 acceptor），放寬到 1–3.8 km 之間不改變工作量也不改變品質保證的性質，故無理由變動。影響：無條款變更；本補充作為 AC-V2-19 的比例性依據。
...
| Clarification／本紀錄 §14 | 2026-09-26 | Issue #39 c1 R1 audit routing signal **R-1**（治理 §4.2）；Orchestrator 依治理 §2.4 派工 DA | decision record [`decision-20260926-desktop-representative-marker-density.md`](decision-20260926-desktop-representative-marker-density.md)（**DV-22**）：R-V2-DD-2／DD-3 與 R-V2-RSP-7／RSP-3 合讀的 §3.6-A clarification——全臺視野的標記圖層成員 ＝ 代表測站集合（每個有有效測站的縣恰一個，不受 HOW 裁量）；同時可見由 RSP-7／RSP-3 決定：桌機初始全臺視野 MAY 隱藏代表標記，但**只因**其 44×44 必要可點區與顯示中代表標記碰撞（最小、確定、README 記載、放大即恢復、被隱藏縣仍可經 DD-4／DD-9(a) 選取）；375 依 RSP-7 顯示數不是 oracle；「中間縮放層級」的密度 HOW 不受限；BASE 的 22／22（北部重疊）不是基線。#39 F-1 修正後 MUST 滿足 DV-22 §4.1 並由 R2 核對；#39 R1 對 RSP-7／AC-V2-14 的其餘結論維持。分配不變（§15.2／§15.3 不動；#39 仍為 RSP-1～8、AC-V2-13／14／15 owner）；#36／#37／#38 不重開；#40／#41 不新增分配；SPEC-V2 文字不變（仍 v2.2）。 | 治理 §5.3 第 2 類：clarification，不新增 AC／INV／R／oracle（§4.1 只組合 RSP-3 的 44×44、AC-V2-11 的代表集合、DD-4／DD-9(a) 的可達性等既有 oracle），不改 #39 的 What to build，不改 accepted 語義（OC S-4／S-10、AB-V2-6／8 不變；375 放寬未延伸到桌機、「22 同時可見」未升為 MUST）；不需 acceptor。 |
| Annotation correction／本紀錄 §3.2 DV-11、DV-13 | 2026-09-26 | Issue #40 c1 R1 audit routing signal **R-1**（治理 §4.2；non-blocking）；Orchestrator 依治理 §2.4 派工 DA | decision record [`decision-20260926-spec-5-3-pixel-figure-correction.md`](decision-20260926-spec-5-3-pixel-figure-correction.md)（**DV-23**）：SPEC-V2 §5.3 R-V2-RAD-5 列與 R-V2-MAP-3 列、本紀錄 DV-11 與 DV-13（含 v2.1 比例性補充第 (1)(3) 點）的 km↔px 換算數值更正為 Web Mercator 正確值（1 km：z7 ≈ 0.89、z10 ≈ 7.13、z12 ≈ 28.5 px；375 px 視窗：z12 ≈ 13 km；z11 ≈ 14.3 px、z14 ≈ 3.3 km；3.8 km 於 z10 ≈ 27 px；200 m 於 z10 ≈ 1.4 px），原文以〔DV-23 更正〕標記保留可追溯。DV-13 的裁決（≤ 1 km）與比例性結論不變；SPEC-V2 版本仍 v2.2、§10 加「annotation correction」列與標頭加註。#39／#40 的分配、evidence 與 CLOSURE 不變；#41 與 Spec Integration Audit 以更正後的 Spec 為準。 | 治理 §5.3 第 2 類：非規範儀器註記的數值更正，不改任何 R／AC／INV／oracle／儀器值（1 km、≥ 20 px、≥ 5 km、minZoom 6／maxZoom 12、E、參考點、zoom 7／10 皆不變），不改任何已作出的 PASS／FAIL，不改 accepted 語義（OC S-8／S-11、AB-V2-7／9 不變）；不需 acceptor。 |
