...
- **Subject**：cycle 1 初始 code anchor ＝ `2f34843`（R1 受審 subject）；**cycle 1 targeted correction（F-1）後的 code anchor ＝ `286ee9d`**（BASE `de004f4`..`286ee9d` 為本票全部產物變更；`2f34843`..`286ee9d` 為 correction delta，見下方「Cycle 1 targeted correction」）；本 worklog 為其後的 record-only commit（`doc/governance/**`，Bindings §7 P7）
- **開始／本次更新**：2026-09-26
...
## Cycle 1 targeted correction（R1 F-1；2026-09-26）

- **依據**：R1 audit record `home_work_01/doc/governance/audit/issue-38-c1-r1.md`（VERDICT: BLOCKING (F-1)；DV-20、DV-21 已由 R1 獨立重驗 PASS）；Orchestrator 的 targeted correction 派工（同 branch、同 worklog identity、不結案、之後 R2）。範圍只限 F-1，另依派工允許處理自有文件的 F-4、F-3（worklog 措辭）。F-2、County view 標記 44×24、375 County 脈絡位置已由 R1 交接 #39，本次未動。
- **F-1 修正（HOW；契約內——R-V2-DD-9(d) 允許多邊形不進焦點序）**：`buildCountyLayer` 對每個縣多邊形在每次 `add` 時設 `tabindex="-1"`（Leaflet 每次把圖層加回地圖都重建 path 元素，故綁在 `add`）。明確的 `tabindex="-1"` 使 Chromium 不再因 Leaflet 的 tooltip focus 監聽而把 path 放進 Tab 序；hover 突顯＋縣名、點選選取不變；鍵盤選縣路徑仍是 County 選單（DD-9(a)）。`styles.css` 的 `:focus { outline: none }` 保留，只針對滑鼠點擊造成的焦點（註解改寫說明）。未選擇「保留可聚焦＋加可見焦點與 Enter／Space」的做法：22 個額外 Tab 停駐點對鍵盤使用者沒有 County 選單以外的價值，且選縣後多數縣在視野外。
- **F-4**：`counties.js` 檔頭改為「瀏覽器只從本應用同源取得、無外部請求」；README 縣界段同步，並註明縣形狀只對指標反應、不是 Tab 停駐點。
- **F-3**：上方 V-5 DV-21 (1)(2) 兩段措辭更正——chip 與類別原因是「存在於 DOM」，≥ 1180 px 選縣後在面板可視區外、捲動即見；可見的是 County 狀態行與地圖內告示。面板版面屬 #39，未改。
- **Artifacts（correction delta `2f34843`..`286ee9d`，產物部分）**：`static/app.js`（`buildCountyLayer` 的 `add` 監聽）、`static/styles.css`（註解）、`static/data/counties.js`（檔頭註解）、`README.md`（縣界段）、`tests/test_county_frontend.py`（新增 `test_county_polygons_are_never_keyboard_tab_stops`）、`tests/check_county_browser.py`（新增 Tab-walk 情境，兩個視野）、`doc/acceptance/screenshots/v2/issue-38/*`（重產：72 項結果、network log、截圖、新增 `*-tabwalk-after.png`；#36／#37 回歸結果更新）。
- **Correction verification**（subject `286ee9d`）：
  - **C-1 Tab walk（新增瀏覽器情境，1280 與 375）**：Now mode 22 條縣 path 的 `tabindex` 全為 `-1`。自 `#map` 起連按 Tab 直到焦點離開地圖——全臺視野：停駐點只有 22 個代表標記與縮放按鈕，**0 個縣 path**，每一停駐點的焦點中心可被命中（未被面板或 overlay 遮蔽）；第一個標記停駐點按 Enter 即選取該站並顯示詳情（不是無作用停駐點）。臺中市與花蓮縣縣視野：停駐點只有縮放按鈕（County view 標記本就 `tabindex=-1`，清單是其鍵盤路徑），0 個縣 path、焦點皆可見。之後 hover 顯示縣名＋點選選取、自 `Back to Taiwan` Tab 到清單＋Enter 開詳情仍成立。
  - **C-2 `python tests/check_county_browser.py` → 72/72 PASS**（原 58 項＋Tab-walk 情境 14 項；DV-20 (i)(ii)(iii) 與 DV-21 (1)(2)(3) 全部仍 PASS）；9 段情境 135 個瀏覽器請求，**外部請求 0**。
  - **C-3 回歸**：#36 `check_modes_browser.py` 37/37；#37 `check_refresh_browser.py` 97/97；V1 `check_series_error_visible.py` PASS。
  - **C-4 全套與 CI**：`pytest` **494 passed**（＋1 靜態守衛）；CI run **`36210392148`**（head `286ee9d`）**success**：`494 passed`、`credential scan passed: 648 tracked files`。既有測試的刪除行只有 `check_county_browser.py` 的兩個情境清單（各加上 Tab-walk 情境）。
  - **C-5 mutation（self-verification）**：M11「移除 `tabindex=-1`（還原 F-1）」→ 靜態守衛 1 項 FAIL＋瀏覽器 Tab-walk 情境多項 FAIL；`app.js` 以 sha256 確認還原。
  - **C-6 憑證／不變產物**：`tools.credential_scan` passed；staged diff 42,560 bytes 以 `.env` 比對真金鑰字面 False；evidence 目錄 grep 哨兵金鑰／上游與平台標記／`opendata.cwa.gov.tw`／`Authorization` 無命中；`git diff --stat de004f4 286ee9d` 對 V-8 清單（含 `basemap.js`、vendor、伺服器檔、`app.py`、`data.db`、workflows）仍為空。
  - **限制**：焦點走查在 headless Chromium（DevTools）；其他瀏覽器引擎未測。
...
- **Required**：Formal mandatory independent audit（治理 §4.1；Bindings §5）。本 worklog 的 mutation checks 與瀏覽器檢查皆為 Executor self-verification，**不是**正式 audit。
- **Records**：cycle 1 R1 `home_work_01/doc/governance/audit/issue-38-c1-r1.md`——**VERDICT: BLOCKING (F-1)**（Medium，blocking）；F-2（Low，交接 #39）、F-3（Low，non-blocking，本次更正 worklog 措辭）、F-4（Low，non-blocking，本次更正）；DV-20、DV-21 R1 獨立重驗 PASS。F-1 的 targeted correction 已完成（上節；subject `286ee9d`），**待 R2 scoped closure review**（Orchestrator 派工）。Executor 的「已修正」不是 closure。

## Remaining work

1. **正式 audit**：R2 closure review of F-1（含修正造成的回歸），Orchestrator 派工。
2. **Concerns（交有權角色判斷；Executor 未自行裁決）**：
   ...
   - （R1 處置後的狀態：(a) 已由 R1 以 F-2 交接 #39；(b)(e) 與 County view 標記 44×24 由 R1 交接 #39；(c) R1 §7 判定契約充分、無需 DA 裁決；(d) R1 §6 依 §4.4 核對。本次 correction 未動這些項目。）
