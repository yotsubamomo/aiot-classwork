...
| AC-04 | MVM | PASS | Static checks pass (22 in `test_static_checks.py`): Python layers import no HTTP client (dotted forms incl.), no `opendata.cwa.gov.tw`/`CWA_API_KEY`; SQL only in `weather_query.py`; frontend requests target only `/api/`; both layers import the one shared module. audit 19 F-4→#20 resolved, 20 F-2. **#28**: the frontend scan now recurses `static/` (incl. the new `static/data/basemap.js`), closing #24 F-4; the vendored vector basemap holds no absolute URL / CWA / key and the browser's only data requests are to `/api/` (AC-04(b) intact — no request target whitelisted). `test_static_checks.py` (recursion guard + no-URL), `test_map_frontend.py`. shots `issue-28-*`. |
...
| AC-17 | ENHANCED | PASS | Map centered on Taiwan, six markers visible & zoomable; marker colours == shared-module band per selected day; click info card Region/Date/Min/Max/derived; 4-band legend w/ "derived" caveat; keyless basemap. Manual acceptance + shots `issue-24-map-*`, `issue-24-map-infocard-ac17.png`. audit 24. **#28 re-verified** on the reworked map card: six pill markers coloured by the day's endpoint `colourBand` (legend swatches painted from the same four tokens — colour==band), click/hover shows Region/Date/Min/Max/derived, four-band legend with the caveat, vendored vector basemap with **zero runtime external requests** (network log all same-origin). shots `issue-28-desktop-dark-ac17.png`, `issue-28-desktop-light-ac17.png`, `issue-28-desktop-hover-tooltip.png`; worklog `20260924-taiwan-map-rework`. |
| AC-18 | ENHANCED | PASS | `Select Date` lists seven days ascending; switching recolours markers + updates info cards to that day's endpoint values (open popups refresh too — #24 F-1 fixed R2). shots `issue-24-map-date1/2-ac18.png`, `issue-24-f1-popup-updates-*`. audit 24 R2. **#28 re-verified**: `Select Date` now sits in the map info panel (seven days ascending, default first); switching updates pills/tooltips/panel to that day's endpoint values **without resetting the map view**; seven-day pill-value/band == endpoint parity attached in worklog (H-3). shots `issue-28-desktop-date1-ac18.png`, `issue-28-desktop-date2-ac18.png`. |
| AC-19 | ENHANCED | PASS | R-EN-1 six items each PASS; desktop (≥1024) & 375px screenshots; 375px `scrollWidth == innerWidth` (375); loading/empty/error states each shot. shots `issue-24-desktop-ok-ac19.png`, `issue-24-mobile-375-ac19.png`, `issue-24-state-*`. audit 23/24. **#28 re-verified** on the map-first layout: R-EN-1 six items PASS; desktop (1280) & 375 shots; 375 `document.documentElement.scrollWidth == innerWidth == 375` (no horizontal scroll); loading/empty/error states each shot with the **map card never blank** and `Select Date` still visible & operable (DR-19, P-7b). shots `issue-28-desktop-dark-fullpage-ac19.png`, `issue-28-375-dark-ac19.png`, `issue-28-375-light-ac19.png`, `issue-28-state-loading.png`, `issue-28-state-empty.png`, `issue-28-state-error.png`. **#28 targeted correction (cycle 1)**: R1 blocking F-1/F-2/F-3 (panel/legend occluded markers/tooltips/status at 641–1150px), F-4 (375px wrong-region click) and F-5 (weak init tests) fixed. V-1..V-5 re-measured across 641→1920px: all six pills and tooltips fully visible + interactive at every width, no h-scroll at 375, hit ≥44×44 with each pill selecting its own Region. **V-2 = PASS per DR-21.1 / OR-V2** (marker-vs-basemap: fill OR the ratified 2px border ≥3:1 vs sea/Taiwan-land/context-land in every state; text-vs-fill ≥4.5:1 — full table in the worklog). New shots `issue-28-tablet-768-dark-ac17.png`, `issue-28-desktop-1024-hover-south.png`, `issue-28-desktop-light-hover-tooltip.png`. |
...
| **#18 R2 N-1** | **offline ingestion (`--acquired-at` / sidecar path) does not validate the acquisition-time format — a malformed value (e.g. `--acquired-at yesterday`) is accepted and written to `IngestionMetadata.ingestedAt`** (R-DB-5, DR-17) | **Medium** | **FIXED in #29** (post-baseline Lightweight work item, `doc/governance/decisions/decision-20260924-acquired-at-validation.md` DR-22; worklog `doc/governance/worklog/20260924-acquired-at-validation.md`). A single validator (`ingestion/acquisition_time.py`) now enforces the exact `YYYY-MM-DDTHH:MM:SS+08:00` format at all three sources — CLI `--acquired-at` (no fallback to the sidecar on failure), the sidecar `acquiredAt` field (non-string values rejected, not coerced), and the online `ingestion_timestamp()` output (validated before any write) — failing closed (exit 1, source-named message, no DB/raw/sidecar write). The committed `data.db`/sidecar value (`2026-09-24T02:24:50+08:00`) is valid and byte-identical, so AC-12/AC-24 evidence is unchanged. Closure gated on the **A-4 independent audit** (`doc/governance/audit/issue-29-c1-*.md`). |
...
| #24 F-4 | static CWA-URL/key scan does not recurse `static/vendor/` | Low | **RESOLVED in #28** — `_static_files()` now recurses `static/` (incl. `static/data/`); vendored-Leaflet non-request URL constants whitelisted exactly (no request target allowed). `test_static_checks.py::test_static_scan_is_recursive_over_subdirectories`. |
| #24 F-5 | 375px east-marker popup clipping / sidebar reset / south outline | Low | Map UX detail; no contract clause violated. **F-5(a) tooltip clipping RESOLVED in #28 (cycle-1 correction)** — per-marker tooltip direction + the ≥1180px floating-panel padding / <1180px stacked layout keep all six tooltips fully visible at every width 641→1920 and 375 (re-measured; the earlier "375 & desktop only" note was corrected after #28 R1 F-2). The south-outline note is moot: the project-authored outline is replaced by the vendored MOI county polygons. |
| #28 A-4 R1 | independent audit (`issue-28-c1-r1.md`) returned BLOCKING F-1/F-2/F-4/F-5 + non-blocking F-3/F-6..F-11 + routing signals RS-1/RS-2 | — | **Blocking F-1/F-2/F-3/F-4/F-5 fixed in the cycle-1 targeted correction** (see worklog "Targeted correction"); RS-1→**DR-21.1** (V-2 reading, PASS, no token change), RS-2→**DR-21.2** (masthead lead sentence in-boundary). Low F-6 (legend swatches) / F-8 (double tab stop) / F-9 (light hover shot) also fixed; F-7/F-10/F-11 tracked (SHOULD / outside the V judgement). **Ladder closed: R2 = BLOCKING(N-1 loading-screenshot evidence, `issue-28-c1-r2.md`) → cycle-2 correction (N-1/N-2/N-3) → Alternate Independent Review = AUDIT CLOSURE (`issue-28-c1-alt.md`). Final subject `5136bd2`.** |
| #24 F-6 | `Select Date` label has no automated regression test | Low | **RESOLVED in #28** — `test_map_frontend.py::test_index_has_map_panel_select_date_and_legend` and `test_dashboard.py` assert the `>Select Date</label>` text. |
